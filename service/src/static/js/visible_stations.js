	...
	const VisibleStationsControl = L.Control.extend({
		...
		onAdd: function (map) {
			...
			const container = L.DomUtil.create(
				"div",
				"leaflet-control leaflet-bar leaflet-control-layers custom-stations-list",
			);
			...
			const contentWrapper = L.DomUtil.create(
				"div",
				"content-wrapper",
				container,
			);
			...
			const sortControls = L.DomUtil.create(
				"div",
				"sort-controls",
				contentWrapper,
			);
			...
			const radios = sortControls.querySelectorAll(
				'input[name="sortPrice"]',
			);
			...
			visibleMarkers.forEach((m) => {
				...
				let g95Style =
					this._currentSort === "gasolina_95"
						...
						: "color:#555;";
				let gasAStyle =
					this._currentSort === "gasoleo_a"
						? "font-weight:bold; color:#000;"
						: "color:#555;";

				if (m.eess.gasolina_95)
					pricesHtml += `<span style="${g95Style} margin-right:5px;">G95: ${m.eess.gasolina_95}€</span>`;
				if (m.eess.gasoleo_a)
					pricesHtml += `<span style="${gasAStyle}">GasA: ${m.eess.gasoleo_a}€</span>`;
